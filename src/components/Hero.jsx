...
export default function Hero() {
  ...
  const topics = [
    {
      title: "Hello",
      description:
      "I'm Vitor Teodoro, a Computer Science student at State University of Maringá (UEM).",
    },
    {
      title: "Goals",
      description:
  ...
  return (
    <div className="relative h-screen bg-[#0a0a0c] text-white overflow-hidden flex items-center px-10 md:px-20">
      ...
      <div className="relative z-10 max-w-xl flex flex-col justify-center min-h-[400px]">
        
        <div className={`transition-opacity duration-500 ease-in-out ${isVisible ? "opacity-100" : "opacity-0"}`}>
          
          <h1 className="text-7xl font-semibold mb-6 tracking-wide">
          ...
          {topics[activeIndex].description && (
            <p className="text-gray-400 text-lg leading-relaxed font-light pr-8 min-h-[80px]">
              {topics[activeIndex].description}
            </p>
          )}

          {topics[activeIndex].stack && (
            <div className="grid grid-cols-4 gap-6 mt-8 animate-fade-in-up">
              {topics[activeIndex].stack.map((tech, index) => (
                <div key={index} className="group relative flex flex-col items-center">
                  <div className="p-3 bg-gray-800/50 rounded-xl border border-gray-700 hover:border-white/50 transition-all duration-300 hover:scale-110 hover:bg-gray-700/80 cursor-pointer">
                    <tech.Icon
      ...
      <div className="absolute right-10 top-1/2 transform -translate-y-1/2 flex flex-col gap-3 z-20 items-end">
        {topics.map((_, index) => (
          <div
            key={index}
          ...
          >
            {index === activeIndex ? (
              <div className="h-[3px] w-10 bg-gray-800 rounded overflow-hidden">
                <div
              ...
              <div className="h-0.75 w-8 bg-gray-700 rounded transition-all duration-300 group-hover:bg-gray-400 group-hover:w-8"></div>
            )}
          </div>
        ))}
      </div>
